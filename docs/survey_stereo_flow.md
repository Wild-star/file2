...
按「非双目领域模块可直接融合」策略，先融合光流领域 U²Flow 的逐像素不确定性引导损失
（`step5_uncertainty.py`，同架构/种子/120 步）：

| 配置 | 模块来源 | EPE末(px) | bad1px | 结论 |
|---|---|---|---|---|
| `fusion`（固定系数 mixture-Laplace） | SEA-RAFT | **1.641** | 57.5% | baseline |
| `fusion_unc`（逐像素 σ 引导损失） | U²Flow (CVPR'26) | 1.689 | 59.7% | **负结果**（−2.9%） |

**诚实分析**：不确定性引导的价值在于真实数据的**不均匀误差分布**（遮挡/无纹理处误差大），
而合成分平滑数据误差均匀、无遮挡，σ 的自适应降权没有发挥空间，反而引入额外的 σ 优化负担
（σ̄ 从 1.16 升到 2.14，却未换来 EPE 下降）。这是 POC 负结果，需在含遮挡的真实数据上复验。

第二个融合：FlowIt 的 **OT 全局匹配初始化**（`step6_ot.py`，同候选数 D=9，同架构/种子/120 步）：

| 配置 | 模块来源 | EPE末(px) | bad1px | 结论 |
|---|---|---|---|---|
| `fusion`（softmax 逐位置相关） | — | **1.361** | 50.2% | baseline |
| `fusion_ot`（Sinkhorn OT + 置信度） | FlowIt (2026) | 2.014 | 65.4% | **负结果**（−48%） |

**负原因（诚实归因）**：① OT 的均匀列约束（列和 = N/D）与真实视差的**偏态分布**不匹配，
在 1/8 粗粒度 + 9 候选下强制均匀反而扭曲匹配（初始 EPE 16.79 vs 12.52 即已体现）；
② 无 dustbin，遮挡/低置信位置无法「吸收」匹配质量，置信度 conf̄≈0.18 接近均匀；
③ OT 的全局一致优势需要更细粒度（1/4 或 1/2）+ 更大候选空间才能体现。

**经验（更新）**：连续两个光流模块（不确定性引导、OT 全局匹配）直接融合均为负结果，说明
**跨域迁移不等于直接替换**——需要①数据分布匹配（合成分无遮挡/无重复纹理，恰是这两个模块
的价值场景）②框架适配（OT 需 dustbin + 细粒度；σ 需用于 gate 而非损失）。POC 的负结果
不证明模块无用，只证明「在当前合成分 + 当前粗粒度设置下不适用」；下一步应在**含遮挡的
真实/合成数据**上重做，或改进适配方式（OT 加 dustbin、σ 引导 token gate）。

### 5.5 方法学修复：单种子不可复现问题（重要）

**发现**：`step7_difficulty.py` 的难度扫描与 `step6_ot.py` 出现结论矛盾（同一「OT vs fusion」
对照，step6 OT=−48%，step7 easy OT=+25%）。根因是**方法学缺陷**：模型在 `train_one` 外构造，
`torch.manual_seed` 在内部才调用 → **模型初始权重每次运行不同**（PyTorch 默认 seed 随机）。

**影响**：step3~step7 的所有对照均为单种子、单初始化，**结论可能不可复现、不可靠**。

**修复**（`step8_multiseed.py`）：① 数据固定 seed 生成（跨种子同一数据）；② 模型在
`train_one` 内、`torch.manual_seed(seed)` 之后构造（可复现）；③ 每模型跑 3 种子，报告
mean±std。**所有后续消融/对照必须以多种子 mean±std 为准，单种子结果仅作方向性参考。**

**多种子重测结果**（80 步，3 种子，easy=平滑无遮挡 / hard=重复纹理+遮挡）：

| 难度 | fusion (baseline) | ot (Sinkhorn OT) | unc (σ 引导) |
|---|---|---|---|
| easy | 2.203±0.123 | 3.903±1.644（**−77.1%**） | 2.065±0.508（+6.3%） |
| hard | 5.680±1.140 | 5.783±1.542（−1.8%） | 9.579±1.116（**−68.7%**） |

**诚实结论**：① **「难度相变」假设不成立**——OT/σ 在 hard 上均未转正，σ 反而显著更差；
② OT 在 easy 上稳健更差（−77%）、在 hard 上约等于 fusion（劣势收窄但不转正）；③ OT 的
std 巨大（±1.6），证实其对初始化**高度敏感**，此前 step6/step7 的 ±48%/±25% 矛盾正是
初始化运气；④ σ 引导仅 easy 微弱 +6.3%（std 大、不稳健），hard 上显著更差。
**两个光流模块的直接迁移，在多种子下均无稳健正增益。**

### 5.6 三个差异点的多种子消融（`step9_multiseed_ablation.py`，80 步，3 种子配对）

| 配置 | EPE mean±std | Δ vs fusion |
|---|---|---|
| fusion（全模块） | 2.203±0.123 | baseline |
| fusion_nosparse（去 token 稀疏） | 2.350±0.271 | +0.146 |
| fusion_noanchor（去相关锚） | 2.962±0.966 | +0.758 |

**逐种子配对发现（关键）**：均值上的 Δ>0 看似「稀疏/锚有益」，但**存在种子反转**——
token 稀疏在 seed1 反而有害（nosparse 2.024 < fusion 2.352）；锚在 seed1 也略有害
（noanchor 2.271 < fusion 2.352），且 noanchor seed2 出现离群值 4.328 主导了均值。
**结论：step4 单种子的「token 稀疏贡献最大（+49%）」是初始化运气，多种子下不稳健。**

### 5.7 深度结论：当前 POC 层面无可稳健主张的模块增益

诚实汇总全部多种子证据：① 光流模块（OT、σ 引导）迁移无稳健正增益；② 三个差异点
（token 稀疏、锚）增益不稳健（种子反转）；③ 核心 novelty 撞车 WAVE-Stereo。
**根本原因**：合成分 + 小模型 + 80 步短训练的 EPE 噪声（±0.1~1.0）与模块增益同量级，
任何单种子结论都被淹没。**论文的可稳健主张只能落在两点**：(a) 方法学警示——立体匹配
