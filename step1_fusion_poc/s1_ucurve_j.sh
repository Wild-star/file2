...
export CKPT_USE="${CKPT_USE:-step1_joint.pth}"
...
for on in 0 1; do
  for it in 1 2 3; do
    ...
    tag="j_on${on}_it${it}"
    PYTHONPATH="/tmp/s1eval:$PWD" ANC_ON=$on ANCHOR_CKPT=$CKPT_USE \
      timeout 900 $MM run -n waft-stereo python main.py --num-gpus 1 --eval-only \
        --config-file "$cfg" --ckpt ckpts/SynLarge/DAv2S-4.pth > "/tmp/s1j_$tag.log" 2>&1 || true
    r1=$(grep -A3 "results for eth3d_nonocc" "/tmp/s1j_$tag.log" | grep "copypaste: 0" | sed 's/.*copypaste: //')
    r2=$(grep -A3 "results for eth3d_all" "/tmp/s1j_$tag.log" | grep "copypaste: 0" | sed 's/.*copypaste: //')
    tt=$(grep -oE "Total inference time: [0-9:.]+" "/tmp/s1j_$tag.log" | head -1 | sed 's/.*time: //')
